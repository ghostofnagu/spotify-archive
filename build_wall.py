...
/* hero: original perspective stream — adapted for the archive's own artwork */
...
.hero-stage { position:absolute; z-index:1; inset:0; container-type: inline-size; perspective: 1100px; }
.stream-card { position:absolute; left:50%; top:57%; width:clamp(68px, 16cqw, 220px); aspect-ratio:1; overflow:hidden; border-radius:3px; background:var(--line);
  transform-origin:center center; box-shadow:0 20px 38px var(--shadow2), 0 4px 11px var(--shadow); will-change:transform,opacity; }
...
@media (hover:hover) { .hero:hover .stream-card { animation-play-state:paused; } }
@media (max-width:700px) {
  ...
  .stream-card { top:63%; width:clamp(56px, 23cqw, 130px); }
}
@media (prefers-reduced-motion: reduce) { .stream-card { transition:none !important; } }
...
// hero: two perspective rails of the archive's own artwork
const heroItems = ALBUMS.filter(a => a.art).sort((a,b) => b.lt - a.lt).slice(0, 36);
const hero = document.getElementById('hero');
const streamCards = [];
const streamCount = Math.min(18, heroItems.length);
for (const side of [-1, 1]) {
  for (let i = 0; i < streamCount; i++) {
    const album = heroItems[(i * 3 + (side === 1 ? 5 : 0)) % heroItems.length];
    const el = document.createElement('div');
    el.className = 'stream-card';
    el.innerHTML = `<img loading="eager" src="${album.art}" alt="">`;
    hero.appendChild(el);
    streamCards.push({ el, side, phase: i / streamCount });
  }
}
let streamPaused = false;
const reducedStream = window.matchMedia('(prefers-reduced-motion: reduce)');
function renderStream(now = 0) {
  const progress = reducedStream.matches ? 0 : (now / 46000) % 1;
  streamCards.forEach(({el, side, phase}) => {
    const t = (phase + progress) % 1;
    const near = Math.pow(t, .72);
    const x = side * (2 + Math.pow(t, 1.72) * 59);
    const y = (1 - t) * 26 - 10;
    const scale = .16 + near * 1.12;
    const tilt = side * (1 - t) * 17;
    const opacity = Math.min(1, t * 7, (1 - t) * 8);
    el.style.transform = `translate(-50%, -50%) translate3d(${x}cqw, ${y}px, 0) scale(${scale}) rotateY(${tilt}deg)`;
    el.style.opacity = opacity.toFixed(3);
    el.style.zIndex = String(Math.round(t * 100));
  });
  if (!reducedStream.matches && !streamPaused) requestAnimationFrame(renderStream);
}
hero.addEventListener('pointerenter', () => { streamPaused = true; });
hero.addEventListener('pointerleave', () => { if (streamPaused) { streamPaused = false; requestAnimationFrame(renderStream); } });
reducedStream.addEventListener?.('change', () => requestAnimationFrame(renderStream));
