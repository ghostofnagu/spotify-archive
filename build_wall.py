...
<title>Permanent Record — Made Yours</title>
<style>
:root, [data-theme="light"] { color-scheme: light;
  --bg:#e9e7e2; --panel:#f4f2ee; --hero-bg:#f4f2ee; --hero-edge:rgba(233,231,226,.62); --ink:#141412; --dim:#7c7a72; --line:#d4d1c9;
  --acc:#1db954; --chip:#fff; --gloss:rgba(255,255,255,.32); --shadow:rgba(20,20,18,.16); --shadow2:rgba(20,20,18,.30); }
[data-theme="dark"] { color-scheme: dark;
  --bg:#0d0d0f; --panel:#16161a; --hero-bg:#000; --hero-edge:rgba(0,0,0,.98); --ink:#ececec; --dim:#85858c; --line:#232327;
...
.hero { position: relative; isolation: isolate; min-height: min(640px, 58vw); margin: 22px 0 8px; overflow: hidden;
  background: var(--hero-bg); border-top: 1px solid var(--line); border-bottom: 1px solid var(--line); }
.hero::before { content:""; position:absolute; inset:0; z-index:0; pointer-events:none;
  background: radial-gradient(ellipse 52% 54% at 50% 51%, transparent 0 45%, var(--hero-edge) 100%); }
...
.hero-caption { position:absolute; z-index:3; left:50%; bottom:clamp(24px, 4vw, 52px); width:min(320px, calc(100% - 48px)); transform:translateX(-50%); margin:0; text-align:center; color:var(--dim); font-size:13px; line-height:1.38; pointer-events:none; }
...
@media (max-width:700px) {
  ...
  .hero-caption { bottom:28px; width:min(290px, calc(100% - 32px)); }
}
...
<section class="hero" aria-label="A moving stream of artist portraits">
  <div class="hero-copy">
    <div class="smallcaps">Permanent Record</div>
    <h2>What you love.<br>Made yours.</h2>
  </div>
  <div class="hero-stage" id="hero" aria-hidden="true"></div>
  <p class="hero-caption">The music that stayed with you, streaming through one personal shelf.</p>
</section>
...
<nav id="archive-nav">
  ...
  <input type="search" id="q" placeholder="Search your record…" autocomplete="off">
...
<footer class="archive-footer">
  <div>
    <div class="smallcaps">Permanent Record</div>
    <h1>Listening is personal. <em>Ownership should be too.</em></h1>
  </div>
  <div class="footer-bottom smallcaps">
    <p>Your listening life, gathered in one place · preview what stayed · find a release to keep</p>
